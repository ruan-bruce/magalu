...
def calcular_preco_minimo(custo, frete, margem_desejada):
    margem_frac = margem_desejada / 100
    denominador = 1 - comissao_magalu - margem_frac
    preco_min = (custo + frete + tarifa_fixa) / denominador
...
def gerar_tabela(custo, margem):
    ...
    for estado, frete in estados.items():
        ...
        margem_liq = (lucro / preco_min) * 100
        dados.append({
            "Estado": estado,
            "Frete (R$)": frete,
...
def main():
    ...
    margem = st.slider("Margem Líquida Desejada (%):", min_value=1, max_value=50, value=30)
    ...
    st.dataframe(df)  # Aqui só passamos o df simples
